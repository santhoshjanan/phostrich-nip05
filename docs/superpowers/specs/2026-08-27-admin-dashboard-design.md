...
- **`/admin`** — server `load` runs two live queries:
  - Staleness: `identifiers` where `status = 'claimed'` and the shared `staleIdentifierCondition()` is true, sorted oldest-first. The condition is PostgreSQL calendar arithmetic: `last_identified_at < now() - interval '6 months'`.
